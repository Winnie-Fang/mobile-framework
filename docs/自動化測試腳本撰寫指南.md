...
**架構設計：**
```python
Navigator().ios.zh.ios_login_page.login_btn().click()
```
...
class NavigatoriOSZh:
    def __init__(self, role=None):
        self.role = role
        self.__ios_otp_page_mu = {}  # 多裝置頁面物件快取
...
# 多裝置使用
navigator = Navigator().ios.zh
navigator.ios_otp_page_mu("old").login_btn().click()  # 舊裝置
navigator.ios_otp_page_mu("new").login_btn().click()  # 新裝置
```
...
def test_multi_device():
    # 裝置1操作
    navigator1 = Navigator().ios.zh  # 使用 defaultRole
    navigator1.ios_login_page.login_btn().click()
    
    # 裝置2操作
    navigator2 = Navigator().ios.zh  # 自動切換到第二個裝置
    navigator2.ios_login_page.login_btn().click()
```
