...
- locator2.md 僅用於規範「定位策略與 selector 撰寫方式」，不得影響最終輸出格式。
- 最終輸出格式必須完全符合「Appium 元素定位規範 v1.1」所定義的 JSON 結構。
- locator2.md 不得定義、修改或簡化 JSON 結構（例如不可輸出 { "<頁面>": [...] } 形式）。
- locator2.md 不得定義輸出檔名、輸出路徑或寫檔時機，僅遵循 v1.1 規範第 9 章。
...
* 檔名：`locator_output_YYYY-MM-DD_HH-mm.json`
* 路徑：`/mobile-framework/mcp_tool/locator/`
* 檔案內容必須與回覆 JSON 完全一致
