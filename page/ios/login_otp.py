...
class iOSOTOPage(BaseObject):
    def __init__(self):
        super().__init__()
        self.set_remark("驗證網銀交易")
    ...
    def otp_title(self):
        return BasicComponent(
            lambda: WebDriverWait(self.driver, 10).until(EC.visibility_of_element_located(
                (AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeStaticText[`name == "啟用企業行動密碼"`][2]'))),
    ...
    def bio_check_title(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, '啟用生物辨識'),
    ...
    def verify_phone_num(self):
        return BasicComponent(
            lambda: WebDriverWait(self.driver, 10).until(EC.presence_of_element_located((AppiumBy.IOS_CLASS_CHAIN,
                                                                                         '**/XCUIElementTypeStaticText[`name == "test 企業戶 ID / 統編：6514***4 手機號碼：0986****89"`]'))),
    ...
    def more_setting_btn(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.IOS_CLASS_CHAIN,
                                             '**/XCUIElementTypeStaticText[`name == "更多設定"`]'),
    ...
    def set_otp(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, '設定企業行動密碼'),
