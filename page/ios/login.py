...
class IhaveLoginPage(BaseObject):
    ...
    def login_logo(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "login_appIcon"),
    ...
    def password_input(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.XPATH, '//XCUIElementTypeSecureTextField[@value]'),
    ...
    def eyes_close(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "password eye close"),
            remark=f"{self.remark()} > 密碼遮蔽鈕(關閉)"
        )

    def eyes_open(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.ACCESSIBILITY_ID, "password eye open"),
            remark=f"{self.remark()} > 密碼遮蔽鈕(開啟)"
        )

    def login_btn(self):
        return BasicComponent(
            lambda: self.driver.find_element(AppiumBy.IOS_CLASS_CHAIN, '**/XCUIElementTypeButton[`name == "登入"`]'),
