...
class iOSOverviewPage(BaseObject):
    ...
    def overview_title(self):
        return BasicComponent(
            lambda : WebDriverWait(self.driver,30).until(
                EC.visibility_of_element_located((AppiumBy.IOS_UIAUTOMATION,'**/XCUIElementTypeStaticText[`name == "總覽"`]'))
            ),
    ...
    def to_do_list(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,"處理清單"),
            remark=f"{self.remark()} > 處理清單"
        )

    def to_do_trade(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,'待覆核交易'),
            remark=f"{self.remark()} > 待覆核交易"
        )
    def recent_reserve(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,'近 1 年預約'),
            remark=f"{self.remark()} > 近一年預約"
        )
    def under_review(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,'送審中交易'),
            remark=f"{self.remark()} > 送審中交易"
        )
    def account_overview(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,'帳戶摘要'),
            remark=f"{self.remark()} > 帳戶摘要"
        )
    # ================================================= 總覽下方功能列 =================================================
    def menu_bar(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.ACCESSIBILITY_ID,'標籤頁列'),
            remark=f"{self.remark()} > 總覽下方功能列"
        )
    def overview(self):
        return BasicComponent(
            lambda : self.driver.find_element(AppiumBy.IOS_CLASS_CHAIN,'**/XCUIElementTypeButton[`name == "總覽"`]'),
