...
def test_open_two_devices():
    ...
    navigator.keyboard.four.click()
    time.sleep(0.5)
    navigator2 = Navigator().ios.zh
    navigator2.ios_login_page.login_slogan().assert_visible()
    assert navigator2.ios_login_page.id_input().is_ready() is True
    time.sleep(1)
    navigator2.ios_login_page.id_input().click()
    navigator2.ios_login_page.id_input().clear()
    # 數字鍵盤輸入
    navigator2.keyboard.number_btn.click()
    navigator2.keyboard.six.click()
